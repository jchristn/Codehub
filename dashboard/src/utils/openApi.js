...
export const FALLBACK_OPERATIONS = [
  ...
  { id: 'getCustomActions', tag: 'Custom Actions', method: 'GET', path: '/v1.0/api/custom-actions', summary: 'List custom actions', parameters: [], requestBody: null, responses: {} },
  { id: 'createCustomAction', tag: 'Custom Actions', method: 'POST', path: '/v1.0/api/custom-actions', summary: 'Create a custom action (agent-agnostic prompt)', parameters: [], requestBody: { content: { 'application/json': { schema: { type: 'object', properties: { name: { type: 'string' }, prompt: { type: 'string' } } } } } }, responses: {} },
  { id: 'runCustomAction', tag: 'Custom Actions', method: 'POST', path: '/v1.0/api/custom-actions/{id}/run', summary: 'Run a custom action in repositories with a chosen agent', parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string' } }], requestBody: { content: { 'application/json': { schema: { type: 'object', properties: { repositoryIds: { type: 'array', items: { type: 'string' } }, agent: { type: 'string', enum: ['claude', 'codex', 'mux', 'opencode'] }, dangerous: { type: 'boolean' }, prompt: { type: 'string' } } } } } }, responses: {} },
